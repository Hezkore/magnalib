...
Function GadgetFromString:TLayoutGadget[]( str:String )
	...
	Local inString:Int
	Local inComment:Int
	Local foundCommentStart:Int
	
	Local extractedKey:String
	Local extractedValue:String
	
	For Local i:Int = 0 Until str.Length
		If Not inComment And str[i] < 32 Then Continue ' Skip junk
		ch = Chr(str[i])
		
		If foundCommentStart Then
			If ch = "/" Then inComment = True
			foundCommentStart = 0
		Else
			If ch = "~q" Then inString = Not inString
			
			If inComment Then
				If ch = "~n" Or ch = "~r" Then inComment = False
